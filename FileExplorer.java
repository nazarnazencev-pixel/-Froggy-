...
public class FileExplorer extends JFrame {
    ...
    private ParsedQuery parseQuery(String raw) {
        ...
        pq.text = parts[0].trim().toLowerCase();
        if (parts.length > 1) {
            pq.extensions = new ArrayList<>();
            for (int i = 1; i < parts.length; i++) {
                String e = parts[i].trim().toLowerCase();
    ...
    private void initUI() {
        ...
        settingsTab.setOnZoomChanged(() -> {
            Dimension savedSize = getSize(); // сохранить
    ...
    private void addHistoryEntry(String query, File searchRoot, List<File> found) {
        String path = searchRoot.getAbsolutePath();
        String home = System.getProperty("user.home");

        String driveLabel;
        if (path.equals(home)) {
            driveLabel = System.getProperty("user.name"); // имя пользователя
        } else {
            driveLabel = path;
        }

        historyEntries.add(0, new HistoryEntry(query + "  [" + driveLabel + "]", found));
        saveSettings();
        rebuildHistoryUI();
    }
    ...
    private void startLiveSearch(){
        ...
        contentPanel.revalidate();
        contentPanel.repaint();
        groupPanels.clear(); groupArrows.clear();
        ...
        String dn = searchRoot.getAbsolutePath();
